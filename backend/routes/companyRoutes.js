...
import { authenticate, authorize } from '../middleware/companyAuth.js';
import User from '../models/User.js';
import Company from '../models/Company.js'; // Add this import

const router = express.Router();

// Public routes - these should NOT have authentication middleware
router.post('/register', registerCompany);
router.post('/login', login);

// OTP verification routes - also public
router.post('/verify-otp', verifyOtp);
router.post('/resend-otp', resendOtp);

// Debug OTP route - public but should be removed in production
router.get('/debug-otp/:email', async (req, res) => {
  try {
    const { email } = req.params;
    const user = await User.findOne({ email });
...
// Debug reset password route - public but should be removed in production
router.post('/debug-reset-password', async (req, res) => {
  try {
    const { email, companyCode, newPassword } = req.body;
...
// Protected routes - require authentication
// This middleware applies to all routes defined AFTER this point
router.use(authenticate);
