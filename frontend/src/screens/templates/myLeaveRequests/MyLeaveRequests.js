...
const MyLeaveRequests = () => {
  ...
  const fetchLeaveStatistics = async () => {
    try {
      const response = await axios.get(`${API_URL}/statistics/${EMPLOYEE.code}`);
  ...
  const getMonthlyChartData = (statistics) => {
    if (!statistics || !statistics.statistics || !statistics.statistics.monthlyUsage) {
      return [];
    }
    
    // Convert the monthlyUsage object to an array of objects
    return Object.entries(statistics.statistics.monthlyUsage).map(([month, days]) => ({
      month,
      days
    }));
  };
  ...
  const getLeaveTypeChartData = (statistics) => {
    if (!statistics || !statistics.statistics || !statistics.statistics.leaveTypeUsage) {
      return [];
    }
    
    // Convert the leaveTypeUsage object to an array of objects
    return Object.entries(statistics.statistics.leaveTypeUsage).map(([type, days]) => ({
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={8}>
                <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
                  <Typography variant="h6" sx={{ mb: 2 }}>Leave Usage by Month</Typography>
                  <Box sx={{ height: 300 }}>
                    {statistics ? (
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <RechartsTooltip />
                          <Legend />
                          <Bar dataKey="days" fill="#8884d8" name="Days Taken" />
                        </BarChart>
                      </ResponsiveContainer>
                    ) : (
                ...
                <Paper sx={{ p: 3, borderRadius: 2 }}>
                  ...
                  {statistics && statistics.upcomingLeaves && statistics.upcomingLeaves.length > 0 ? (
                    <TableContainer>
                      <Table size="small">
                        <TableHead>
                          <TableRow>
                    ...
                    {statistics ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          >
                            {getLeaveTypeChartData(statistics).map((entry, index) => (
              ...
              <Paper sx={{ p: 3, borderRadius: 2 }}>
                <Typography variant="h6" sx={{ mb: 2 }}>Leave Balance Summary</Typography>
                {leaveBalance ? (
                  <Box>
                    {LEAVE_TYPES.map((type) => {
                      ...
                      return (
                        <Box key={type.value} sx={{ mb: 2 }}>
                          ...
                          <Box sx={{ position: 'relative', height: 8, bgcolor: '#eee', borderRadius: 1 }}>
                            <Box
                              sx={{
                                position: 'absolute',
                                left: 0,
      ...
      {tabValue === 1 && (
        <Paper sx={{ p: 3, borderRadius: 2 }}>
          <Typography variant="h6" sx={{ mb: 3 }}>My Leave Requests</Typography>
          {loading ? (
                  ...
                  {LEAVE_TYPES.map((type) => {
                    ...
                    return (
                      <TableRow key={type.value}>
                        ...
                        <TableCell>{balance.total - balance.used - balance.pending}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <TextField
              ...
              fullWidth
              margin="normal"
              helperText={`Available balance: ${getAvailableBalance(formData.leaveType)} days`}
            >
            ...
            {formData.halfDay && (
              <TextField
                ...
                onChange={(e) => handleInputChange('halfDayType', e.target.value)}
                fullWidth
                margin="normal"
              >
            ...
            <Box sx={{ mt: 2 }}>
              <Typography variant="body2" color="textSecondary">
                Number of days: {calculateBusinessDays(formData.startDate, formData.endDate, formData.halfDay)}
              </Typography>
            </Box>
          </Box>
        </DialogContent>
