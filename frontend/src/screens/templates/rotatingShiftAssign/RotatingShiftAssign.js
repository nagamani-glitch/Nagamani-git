...
// Add these API functions at the top of the file after imports
...
const createRotatingShift = async (data) => {
  try {
...
const RotatingShiftAssign = () => {
  ...
  // Add these state variables inside the RotatingShiftAssign component
...
const [currentUserProfile, setCurrentUserProfile] = useState(null);
  ...
  useEffect(() => {
    ...
    socket.on('new-notification', (notification) => {
      ...
      // Reload the rotating shift requests to reflect the changes
...
const fetchCurrentUser = async () => {
  try {
    ...
    if (!userId) {
      setProfileError("No user ID found in localStorage");
      console.error("No user ID found in localStorage");
      setSnackbar({
        open: true,
        message: "User ID not found. Please log in again.",
    ...
    if (response.data.success) {
      ...
      // Set the current user and current user profile
      setCurrentUser(userData);
  ...
  const loadRotatingShiftRequests = async () => {
    try {
      ...
      if (tabValue === 0) {
        ...
        const response = await fetchRotatingShifts();
        const filteredRequests = response.data.filter(
          (request) => request.userId === userId
        );
        setShiftRequests(filteredRequests);
      } else {
        // Review tab - show all requests for HR/admin to review
        const response = await fetchRotatingShifts();
        // Filter for requests that need review
        const requestsForReview = response.data.filter(
          (request) => request.isForReview === true
        );
        setAllocatedShifts(requestsForReview);
      }
    } catch (error) {
      console.error("Error loading rotating shift requests:", error);
      setSnackbar({
...
// Update the bulk operations as well
const handleBulkApprove = async () => {
  try {
    const reviewerName = localStorage.getItem("userName") || "Admin";
...
const handleNotificationClick = (notification) => {
  ...
  if (notification.type === 'rotating-shift') {
    setTabValue(0); // Navigate to My Requests tab
    
    // Mark the notification as read
    markRotatingShiftNotificationAsRead(notification._id);
    
    // Filter the requests to show the one related to the notification
    if (notification.relatedId) {
      setSearchTerm(notification.relatedId);
    }
  }
};
  ...
  //     const response = await createRotatingShift(requestData);
  //     setShiftRequests((prev) => [...prev, response.data]);
  //     setCreateDialogOpen(false);
  //     resetFormData();
  //     setSnackbar({
  //       open: true,
  //       message: "Rotating shift request created successfully",
  //       severity: "success",
  //     });
  //   } catch (error) {
  //     console.error("Error creating rotating shift request:", error);
  //     setSnackbar({
  //       open: true,
  //       message: "Error creating rotating shift request: " + error.message,
  //       severity: "error",
  //     });
  //   }
  // };
...
const handleCreateShift = async () => {
  try {
    ...
    if (!formData.requestShift || !formData.requestedDate || !formData.requestedTill) {
      setSnackbar({
        open: true,
    ...
    if (new Date(formData.requestedDate) > new Date(formData.requestedTill)) {
      setSnackbar({
        open: true,
        message: "Requested date cannot be after requested till date",
        severity: "warning",
      });
      return;
    }
    ...
    const requestData = {
      ...
      employeeCode: formData.employeeCode,
      requestedShift: formData.requestShift,
      currentShift: currentUserProfile?.joiningDetails?.shiftType || "Regular Shift",
      requestedDate: formData.requestedDate,
      requestedTill: formData.requestedTill,
      description: formData.description || "",
      isPermanentRequest,
    ...
    setSnackbar({
      open: true,
      message: "Error creating rotating shift request: " + errorMessage,
      severity: "error",
    });
  }
};
            ...
            <TableRow>
              <TableCell colSpan={tabValue === 1 ? 10 : 9} align="center" sx={{ py: 4 }}>
                <Typography variant="body1" color="text.secondary">
                  No rotating shift requests found matching your filters.
                </Typography>
                <Button
                  variant="text"
                  color="primary"
  ...
  return (
    ...
    >
      <Box>
        ...
        {/* Tabs for My Requests and Review */}
        <Tabs
          value={tabValue}
        ...
        <DialogContent sx={{ padding: "32px", backgroundColor: "#f8fafc" }}>
          {loadingProfile ? (
              ...
              >
                ...
                >
                  ...
                  <Typography variant="body2">
                    ...
                    {currentUserProfile.joiningDetails?.shift ||
                      "Regular Shift"}
                  </Typography>
                </Box>
              </Paper>
            ...
            >
              ...
              <MenuItem value="Flexible Shift">Flexible Shift</MenuItem>
              <MenuItem value="Weekend Shift">Weekend Shift</MenuItem>
            </TextField>
