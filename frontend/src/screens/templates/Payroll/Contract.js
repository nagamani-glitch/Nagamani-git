...
const Contract = () => {
  ...
  const fetchContracts = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/payroll-contracts");
      console.log("API Response:", response.data);
      
      const contractsData = Array.isArray(response.data) ? response.data : response.data.data;
  ...
  const handleApplyFilter = () => {
    const newFilteredContracts = contracts.filter((contract) => {
      ...
      const matchesContractStatus = !filterData.contractStatus || 
        contract.contractStatus?.toLowerCase() === filterData.contractStatus.toLowerCase();
      const matchesStartDate = !filterData.startDate || 
        contract.startDate >= filterData.startDate;
      const matchesEndDate = !filterData.endDate || 
        contract.endDate <= filterData.endDate;
  ...
  const handleSelectAll = () => {
    setSelectedContracts(selectAll ? [] : filteredContracts.map(contract => contract._id));
    setSelectAll(!selectAll);
  };
  ...
  const handleExportSelected = () => {
    ...
    const exportData = selectedData.map(contract => ({
      'Contract Type': contract.contract,
      'Employee': contract.employee,
      'Start Date': contract.startDate,
      'End Date': contract.endDate,
      'Wage Type': contract.wageType,
      'Basic Salary': contract.basicSalary,
      'Filing Status': contract.filingStatus,
    ...
    const fileName = `contracts_export_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <div className="create-page">
        <div className="create-page-sub-container">
          ...
          <form className="create-form">
            ...
            {/* Basic Salary and Filing Status */}
            <div className="form-row">
              <div className="form-group">
                <label>
                  Basic Salary <span className="required">*</span>
            ...
            <div className="form-row half-width">
              ...
              <div className="form-group">
                <label>
                  Calculate Daily Leave Amount
                  <FaInfoCircle className="info-icon" title="Leave amount will be calculated by dividing basic pay by working days" />
                </label>
                <label className="toggle-switch">
                  <input
                    type="checkbox"
            ...
            {/* Form Buttons */}
            <div className="form-row">
              <button type="button" className="contract-save-button" onClick={handleSaveCreate}>
                Save
              </button>
