...
import {
  Box, Typography, Button, Grid, Tabs, Tab, IconButton,
  Paper, Avatar, Divider, InputBase, Dialog, DialogTitle,
                            ...
                            <Box sx={{
                              ...
                              <Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}>
                                {Array.from({ length: 5 }).map((_, idx) => (
                                  <StarIcon key={idx} sx={{
                                    fontSize: 16,
                                    color: idx < candidate.stars ? "#FFD700" : "#E0E0E0"
                                  }} />
                                ))}
                              </Box>
                            </Box>
          ...
          <Box sx={{ mt: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              {Array.from({ length: 5 }).map((_, idx) => (
                <IconButton
                  key={idx}
                  onClick={() => setNewCandidate({ ...newCandidate, stars: idx + 1 })}
                >
                  <StarIcon sx={{
