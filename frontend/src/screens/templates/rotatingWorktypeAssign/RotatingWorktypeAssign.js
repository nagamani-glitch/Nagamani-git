  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (deleteType === "worktype" && itemToDelete) {
        ...
        setSelectedAllocations([]);
        setShowSelectionButtons(false);
        console.log(`${selectedAllocations.length} ${itemToDelete.type} deleted successfully`);
      }
...
<Typography
        ...
        >
          {tabValue === 0 ? "Rotating Work Type Requests" : "Allocated Work Types"}
        </Typography>
...
>
  <Table stickyHeader>
    ...
    <TableBody>
      ...
      {(tabValue === 0 ? worktypeRequests : allocatedWorktypes).length === 0 && (
        <TableRow>
